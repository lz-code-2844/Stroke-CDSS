...
首先根据 CTA 视频评价血管连续性、管腔显影、充盈缺损及远端血流情况，并记录能够支持判断的直接影像征象。
...
## Step 1: 自动分析结果整合
将 cta_tool_raw 作为辅助定位和定量信息，并与直接影像征象进行对照：

- 若两者一致，综合形成闭塞或狭窄结论。
- 若两者不一致，描述差异并评价影像质量，根据现有证据给出“存在、未见或不确定”的结论。
- 证据不足时保留不确定性，并提出复核原始影像或补充血管检查的建议。
...
证据整合：
- 报告应以 Reasoning 阶段记录的直接影像征象为基础，并结合自动分析数据。
- 不同来源不一致时，应说明差异、影像质量及结论置信度。

拒绝模糊用语：
- 若 lvo_detected=是，结论必须使用“存在闭塞 / 可见闭塞 / 明确闭塞”
- 对于 lvo_detected=是的阳性结论，不使用“疑似 / 可能”等不确定措辞
- 若 lvo_detected=不确定，应说明证据限制及建议的复核方式
...
5. **数据差异说明**：
   - 若 Reasoning 提到直接影像征象与自动分析数据不一致，Report 应说明该差异及其对置信度的影响。
   - 若证据不足，允许结论为“不确定”并建议复核。

6. **术语严谨性**：
   - lvo_detected=是的阳性结论中出现"疑似""可能"等模糊词汇 → FAIL
